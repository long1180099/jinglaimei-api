...
// ==================== 图片URL补全工具函数 ====================
// 兼容旧数据中的相对路径图片URL，补全为完整地址
function fixImageUrl(url) {
  if (!url) return '';
  // 已经是完整URL（http/https开头）则不处理
  if (/^https?:\/\//i.test(url)) return url;
  // 相对路径补全：优先使用环境变量，fallback 到生产域名
  const API_BASE = process.env.PUBLIC_URL || 'https://api.jinglaimei.com';
  return API_BASE + url;
}
// 批量修复商品对象中的图片URL
...
router.post('/wx-phone-login', async (req, res) => {
  ...
  try {
    ...
    // 但有更规范的 real_name（后台创建时填写的），则同步 username 为 real_name
    if (user.real_name && user.real_name.trim() !== '') {
      const isAutoGenerated = !user.username ||
        user.username.startsWith('用户') ||
        user.username.startsWith('微信用户') ||
        /^\d+$/.test(user.username) ||
        user.username.startsWith('user');
      if (isAutoGenerated || !user.username || user.username.trim() === '') {
        console.log(`[用户名同步] 用户${user.id}: "${user.username}" → "${user.real_name}" (基于real_name同步)`);
        db.prepare("UPDATE users SET username = ?, updated_at = datetime('now','localtime') WHERE id = ?")
          .run(user.real_name, user.id);
    ...
    return success(res, {
      ...
      userInfo: {
        id: user.id,
        username: user.username || user.real_name || '用户' + phone.slice(-4),
...
router.post('/phone-login', async (req, res) => {
  ...
  // 🔗 用户名同步（与 wx-phone-login 保持一致）：如果 username 是自动生成的但有 real_name，则自动同步
  if (user.real_name && user.real_name.trim() !== '') {
    const isAutoGenerated = !user.username ||
      user.username.startsWith('用户') ||
      user.username.startsWith('微信用户') ||
      ...
      user.username.startsWith('user');
    if (isAutoGenerated || !user.username || user.username.trim() === '') {
      console.log(`[用户名同步-phoneLogin] 用户${user.id}: "${user.username}" → "${user.real_name}"`);
      db.prepare("UPDATE users SET username = ?, updated_at = datetime('now','localtime') WHERE id = ?")
        .run(user.real_name, user.id);
      user.username = user.real_name;
    }
...
router.get('/team', async (req, res) => {
  ...
  // 计算间推人数 = 团队总人数 - 直推人数
  const indirectCount = Math.max(0, teamCount - directCount);

  // 返回字段名对齐小程序端 my-team.js 的读取:
  //   team.total(总人数), team.direct(直推), team.indirect(间推)
  //   members 数组(每个成员用 nickname 字段显示名称)
  const membersWithNickname = members.map(m => ({
    ...m,
    nickname: m.real_name || m.username || '微信用户'
  }));
...
router.get('/team/tree', async (req, res) => {
  ...
  function buildTree(parentId, depth = 0) {
    ...
    return children.map(child => ({
      ...child,
      nickname: child.real_name || child.username || '微信用户',
  ...
  return success(res, {
    ...(me || {}),
    nickname: me ? (me.real_name || me.username) : '微信用户',
    children: buildTree(decoded.id)
  });
  } catch(e) {
...
router.get('/team/ranking', async (req, res) => {
  ...
  const ranking = db.prepare(`
    SELECT u.id, u.username, u.real_name, u.avatar_url, u.agent_level,
           ...
           (SELECT COUNT(*) FROM users sub WHERE sub.parent_id = u.id AND (sub.is_deleted = 0 OR sub.is_deleted IS NULL)) as team_count,
           COALESCE(u.total_income, 0) as total_income
    FROM users u
  ...
  // 补充 nickname 字段对齐前端 my-team.js 的映射
  return success(res, ranking.map(r => ({
    ...r,
    nickname: r.real_name || r.username || '微信用户'
  })));
  } catch(e) {
